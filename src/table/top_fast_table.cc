...
#define ToStr(...) json(std::string(buf, snprintf(buf, sizeof(buf), __VA_ARGS__)))
    ...
    djs["Statistics"] = json::object({
      { "sum_full_key_len", SizeToString(sum_full_key_len_) },
      ...
      { "avg_index_len", SizeToString(size_t(sum_index_len_/1e6/sum_index_num_)) },
      ...
      { "raw:key/all", ToStr("%6.3f%%", sum_full_key_len_*100.0/sumlen) },
      { "raw:val/all", ToStr("%6.3f%%", sum_value_len_*100.0/sumlen) },
      { "zip:key/all", ToStr("%6.3f%%", sum_index_len_*100.0/(sum_index_len_ + sum_value_len_)) },
      { "zip:val/all", ToStr("%6.3f%%", sum_value_len_*100.0/(sum_index_len_ + sum_value_len_)) },
